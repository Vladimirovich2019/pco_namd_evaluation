...
# Task
Based on the baseline CFP and OCT images, predict the patient's anatomic outcome after a 6-month course of anti-VEGF therapy. Use the definitive anatomical criteria below.
...
4. **Response Prediction**: Based on the baseline structural features, reason about the expected 6-month treatment response.
...
# Role
You are an expert ophthalmologist and retinal specialist. Your task is to predict the structural response to a 6-month anti-VEGF treatment course in patients with nAMD.

# Task
You are given the patient's baseline CFP and OCT images, AND the following exact biomarker measurements (graded by a retinal expert). Use these precise measurements together with the images to predict the anatomic outcome after a 6-month course of anti-VEGF therapy.
...
3. **Response Prediction**: Based on the combined biomarker profile and image findings, predict the most likely 6-month treatment response.
...
# Task
You are given the following exact biomarker measurements from a patient with nAMD at baseline. Based solely on these structured data values, predict the anatomic outcome after a 6-month course of anti-VEGF therapy. **You do not have access to images — rely only on the biomarker values provided below.**
...
# Canonical full class names as they appear in the prompt's output format.
MNV_CLASS_NAMES = {
    'Type 1 MNV (Occult)': '1',
...
def _map_mnv_label(x):
    ...
    if x_str in {'1', '2', '3', 'Mix'}:
        return x_str
    if x_str in MNV_CLASS_NAMES:
        return MNV_CLASS_NAMES[x_str]
    # Substring match on the canonical class names, tolerant of the extra text a
    # model may append (e.g. "Type 2 MNV (Classic) - subretinal").
    for name, label in MNV_CLASS_NAMES.items():
        if name.lower() in x_str.lower():
            return label
    # Unrecognized answer: return "" so the case is excluded from the metrics
    # instead of being silently attributed to some class.
    return ""
...
def _map_response_label(x):
    ...
    if x_str in {'1', '2', '3', '4'}:
        return x_str
    # The negative class is matched first and on its own written forms, so that a
    # hedged answer mentioning another class ("not a good response") is not read
    # as that class.
    if x_str in {'non-response', 'nonresponse', 'non response', 'no response',
                 'no-response', 'none', 'non'}:
        return '4'
    if 'good' in x_str:
        return '1'
    if 'partial' in x_str:
...
def evaluate(final_results: list, task: dict):
    ...
    # A case is only "missing" if inference never produced a usable response.
    # An empty prediction string means the model answered but the <prediction>
    # tag could not be parsed — that is an unmappable answer, not a missing run,
    # and must be scored as an error rather than blocking evaluation.
    n_missing = (~df['success'] | df['predicted'].isna()).sum()
    if n_missing > 0:
    ...
    # Only cases with a usable ground truth and a recognized prediction enter the
    # contingency table: missing (never answered) and unrecognized (answered but
    # not parseable into a class) predictions are excluded from all metrics.
    df = df[df['gt'].notna() & (df['gt'] != "")]
    n_unrecognized = int((df['pred'].isna() | (df['pred'] == "")).sum())
    valid = df[df['pred'].notna() & (df['pred'] != "")]
    if len(valid) == 0:
    ...
    cm, _ = compute_confusion_matrix(valid['gt'].tolist(), valid['pred'].tolist(), labels=labels)
    metrics = compute_metrics_from_confusion(cm)
    for label in metrics:
        metrics[label]['support'] = int((valid['gt'] == label).sum())

    # Balanced accuracy: mean per-class recall, insensitive to class imbalance.
    per_class_recall = [metrics[lb]['recall'] for lb in labels if metrics[lb]['support'] > 0]
    balanced_acc = float(np.mean(per_class_recall)) * 100 if per_class_recall else 0.0
    ...
    if n_unrecognized > 0:
        print(f"\nExcluded from metrics: {n_unrecognized} case(s) with a missing or "
              f"unrecognized prediction ({len(valid)} of {len(df)} cases evaluated).")

    print_classification_report(metrics, {'macro': macro, 'weighted': weighted}, names,
