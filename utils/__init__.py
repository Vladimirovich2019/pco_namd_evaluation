...
def compute_confusion_matrix(y_true, y_pred, labels=None):
    """Build a confusion matrix, skipping cases whose prediction was dropped.

    ``y_true`` and ``y_pred`` are expected to be aligned lists. Cases whose
    prediction is missing (``None``) or could not be recognized (``""``) are
    omitted from the contingency table entirely, so that all reported metrics
    share the same denominator.

    Returns ``(confusion_matrix, n_skipped)``.
    """
    if labels is None:
    ...
    for true, pred in zip(y_true, y_pred):
        if true not in cm:
            continue
        if pred is None or pred == "" or pred not in cm[true]:
            n_skipped += 1
            continue
        cm[true][pred] += 1
...
def print_classification_report(metrics, average_metrics=None, class_names=None,
